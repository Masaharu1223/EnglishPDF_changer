...
- [x] ~~Claude APIのモデル選定~~ → **決定: Haiku 4.5**。Sonnet 4.5/5はHaiku 4.5の3倍のコスト($3/$15 vs $1/$5 per 1M tokens)。翻訳品質に不満が出た場合は`ANTHROPIC_MODEL`環境変数のみで切替可能な設計にしている
- [x] ~~OpenAI TTSのモデル選定~~ → **決定: `tts-1`**(標準品質、9ボイス、最安)。短い1文単位の読み上げでは`tts-1-hd`/`gpt-4o-mini-tts`との差が体感しにくいため
- [x] ~~逐次処理のままでよいか、限定並列化が必要か~~ → **決定: 並列化する**(下記「並列処理の方式」参照)
- [ ] 1時間相当のテキストを一度に投入した場合のClaude API側のレート制限(RPM/TPM)に抵触しないか。未確認

## 並列処理の方式

現行の`page.tsx`(line 71)は`for`ループでチャンクを1つずつ`await`する**逐次処理**になっているが、これを**スライディングウィンドウ方式の並列処理**に変更する。

- 同時実行数(ウィンドウサイズ)の目安: 3〜5(Claude APIのレート制限を見ながら実装時に調整)
- 全チャンクを一括送信するのではなく、常にウィンドウサイズ分だけ「送信済み・応答待ち」の状態を維持し、1件完了したら即座に次の1件を投入する(バッチ単位で待つ方式ではない)
- 実装上はNode.jsの非同期I/O(イベントループ)による並行処理であり、OSレベルのマルチスレッド・マルチプロセスではない(こちら側はレスポンス待ちをしているだけで、実際の計算はClaude API側で行われる)
- ライブラリ例: `p-limit`等で同時実行数を制御する実装が一般的

この内容に伴い、Technical Risksの「現行の逐次処理のままだと...」の行は解消見込みとして扱う。
...
- **サーバー側での利用回数制限(公開時のコスト保護)**: 現状はローカル環境で自分のAPIキーを使うのみのため急ぎではないが、将来ポートフォリオとして誰でもアクセスできる形で公開する場合、訪問者の利用もすべて開発者自身のAPIキーで課金される。既存の(未コミットの)無料/有料ティアの利用回数制限はlocalStorageのみで管理されておりブラウザの開発者ツールで容易に回避できるため、公開前にサーバー側での利用回数制限(レート制限)を別途実装する必要がある(会話内で確認済み)

## 検討したが不採用にした選択肢

- **音声/動画アップロード時のローカルWhisper文字起こし(whisperkit-cli)**: YouTube文字起こしの精度向上を狙い、`brew install whisperkit-cli`(Apple Silicon向けWhisper実装、large-v3モデル)を実際に試し、Chrome拡張機能による文字起こしより明らかに高精度な結果(句読点の正しい区切り、固有名詞の認識精度向上)を確認した。しかし、このアプリ自体に組み込む選択肢としては**不採用**とした。理由:
  1. **デプロイ不可**: whisperkit-cliはmacOS/Apple Silicon専用(CoreML、Neural Engine前提)であり、本アプリのデプロイ先であるVercel(Linuxサーバーレス環境)では動作しない
  2. **処理時間**: 実測で43分の動画の文字起こしに30分以上かかった。Vercelのサーバーレス関数の実行時間上限に収まらず、「投稿から1分以内」という本PRDの目標とも根本的に矛盾する
  3. **アーキテクチャの一貫性**: Ollama(ローカル前提)をClaude APIに置き換えた今回の方針転換の動機(環境非依存・速度・安定性)と真っ向から矛盾する選択になる

  もし将来アプリに音声/動画アップロード機能(文字起こし)を追加する場合は、Vercel上で動作するクラウドAPIである**OpenAI Whisper API**が現実的な代替候補になる(TTSで既にOpenAI APIを採用している方針とも一貫する)。ただし従量課金となる点は考慮が必要。
  なお、個人のワークフローとして「手元でwhisperkit-cliを使い、生成したテキストファイルを手動でアプリにアップロードする」という使い方自体は、既存の「YouTube URLからの自動取得はスコープ外、テキスト貼り付けで対応する」という決定と矛盾せず、引き続き有効な手段である。
...
|------|------------|------------|
| ~~現行の逐次処理のままだと長尺コンテンツで完了まで数分かかる~~ | - | **解消済み方針**: スライディングウィンドウ方式の並列処理を採用することで決定(「並列処理の方式」参照) |
| 1時間相当のテキストを短時間に大量チャンクでリクエストした場合、Claude APIのレート制限に抵触する可能性 | L〜M | 未検証。並列処理の同時実行数(3〜5想定)を調整弁として使う。実装時にレート制限のドキュメントを確認し、必要ならリトライ/バックオフを実装 |
